...
@SpringBootTest(
    properties = {"topic1-to-send-message=some-test-topic1",
        "topic1-to-consume-message=some-test-topic",
...
class SuccessfulPaymentTest extends DatabaseSuite {
  ...
  public void shouldPaidSucceed() throws JsonProcessingException, InterruptedException {
    Purchase purchase = new Purchase(500, 1L);
    when(purchaseRepository.findById(1L)).thenReturn(Optional.of(purchase));
    when(purchaseRepository.save(any())).thenReturn(purchase);
    CompletableFuture<SendResult<String, String>> sendResult = kafkaTemplate.send("some-test-topic",
        objectMapper.writeValueAsString(new BookPurchaseMessage(1L, 1L)));
    Thread.sleep(15000);
    KafkaTestConsumer consumer = new KafkaTestConsumer(KAFKA.getBootstrapServers(), "book-service-group");
    ...
    records.iterator().forEachRemaining(
        record -> {
          ...
          assertEquals(new BookPurchaseResult(1L, "paid"), message);
        }
    );
  }
}